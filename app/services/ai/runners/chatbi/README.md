...
| 模块 | 职责 |
|------|------|
| `run_state.py` | `DataRunState`（`_DataRunState` 别名）运行态 |
...
| `tool_gate_wrapper.py` | Schema / SQL 重复等工具层 Gate |
| `tool_result_handlers.py` | Schema/SQL 工具结果解析与应用 |
| `sql_gates.py` | SQL 静态风险、schema 预检、Gate 检测 |
