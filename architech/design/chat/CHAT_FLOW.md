...
## 6. 本地执行器与 Runner（General / ChatBI）

薄 Executor 仅转发到 Runner；实际 ReAct 由 **AgentScope Agent** 驱动。详见 [../AGENTSCOPE_RUNTIME.md](../AGENTSCOPE_RUNTIME.md)。

### 6.1 GeneralChatExecutor → GeneralAgentRunner

| 文件 | 职责 |
|------|------|
| `app/services/ai/executors/chat_executor.py` | 薄封装，调用 Runner |
| `app/services/ai/runners/general_agent_runner.py` | 消息准备、工具解析、事件流 |
...
### 6.2 DataQueryExecutor → DataAgentRunner
...
在 AgentScope ReAct 之上叠加 `_DataRunState` 护栏；事件映射与 General 共用 `event_stream.py`。
...
| Runner（General / ChatBI） | `app/services/ai/runners/general_agent_runner.py`、`data_agent_runner.py` |
