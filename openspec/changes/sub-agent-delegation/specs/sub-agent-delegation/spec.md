...
系统必须支持在工具注册表中注册名为 `sub_agent_call` 的委托工具，且该工具默认对主助手启用。
...
子代理在执行委托任务时，系统必须为其分配一个独立的、干净的消息历史上下文，不能掺杂主智能体当前会话的无关上下文。
...
在委托调用期间，子代理在后端产生的所有执行日志（`type: "log"`）必须被捕获，并在重写标识（以指明来源智能体）后实时穿透并发送至主会话的前端流式 SSE 中。
...
子代理执行完成后，其输出中的多模态、卡片标签等干扰文本必须被剥离或转换，然后再将纯文本或结构化数据返回给主助手。
